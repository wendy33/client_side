...
public class ParentCategoryTether {
	...
	public String getParentCategoryNameFromID(Context context,
			int parentCategoryID) {
		Cursor result = helpDataCursor(context, new String[] { Keys.ParentCategory.NAME }, Keys.ParentCategory.ID + "=?",new String[] { Integer.toString(parentCategoryID) });
		// DatabaseHandler db = new DatabaseHandler(context);
		// Cursor result = db.queryFromTable(DatabaseHandler.PARENT_CATEGORY,
		// new String[] { Keys.ParentCategory.NAME },
		// Keys.ParentCategory.ID + "=?",
		// new String[] { Integer.toString(parentCategoryID) });
		result.moveToFirst();
		return result
				.getString(result.getColumnIndex(Keys.ParentCategory.NAME));
	}
	...
	public List<String> getParentCategories(Context context) {
		Cursor result = helpDataCursor(context, new String[] { Keys.ParentCategory.NAME }, null, null);
//		DatabaseHandler db = new DatabaseHandler(context);
//		Cursor result = db.queryFromTable(DatabaseHandler.PARENT_CATEGORY,
//				new String[] { Keys.ParentCategory.NAME }, null, null);
		List<String> parentCategories = new ArrayList<String>();
		while (result.moveToNext()) {
			parentCategories.add(result.getString(result
					.getColumnIndex(Keys.ParentCategory.NAME)));
		}
	...
	public Map<String, Integer> getParentNameToIDMap(Context context) {
		Cursor result = helpDataCursor(context, new String[] {
				Keys.ParentCategory.NAME, Keys.ParentCategory.ID }, null, null);
//		DatabaseHandler db = new DatabaseHandler(context);
//		Cursor result = db
//				.queryFromTable(DatabaseHandler.PARENT_CATEGORY, new String[] {
//						Keys.ParentCategory.NAME, Keys.ParentCategory.ID },
//						null, null);
		Map<String, Integer> parentNameToID = new HashMap<String, Integer>();
		while (result.moveToNext()) {
			String parentName = result.getString(result
					.getColumnIndex(Keys.ParentCategory.NAME));
			int ID = result.getInt(result
					.getColumnIndex(Keys.ParentCategory.ID));
			parentNameToID.put(parentName, ID);
		}
		return parentNameToID;
	}
	
	public Cursor helpDataCursor(Context context, String[] columnNames, String whereClause, String[] whereArgs){
		DatabaseHandler db = new DatabaseHandler(context);
		Cursor result = db
				.queryFromTable(DatabaseHandler.PARENT_CATEGORY, columnNames, whereClause, whereArgs);
		return result;
	}

	public void deleteParentCategory(Context context, int ID) {
		DatabaseHandler db = new DatabaseHandler(context);
		db.deleteEntries(DatabaseHandler.PARENT_CATEGORY, Keys.SubCategory.ID
				+ "=?", new String[] { Integer.toString(ID) });
	}

	public String getTimeOfLastUpdate(Context context) {
		return TetherUtils.getTimeOfLastUpdate(context,
				DatabaseHandler.PARENT_CATEGORY);
	}

	public void setTimeOfLastUpdate(Context context, String time) {
		TetherUtils.setTimeOfLastUpdate(context,
				DatabaseHandler.PARENT_CATEGORY, time);
	}
}
