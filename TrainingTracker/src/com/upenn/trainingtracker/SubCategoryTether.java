...
public class SubCategoryTether {
	...
	public Cursor helpDataCursor(Context context, String[] columnNames,
			String whereClause, String[] whereArgs) {
		DatabaseHandler db = new DatabaseHandler(context);
		Cursor result = db.queryFromTable(DatabaseHandler.SUB_CATEGORY,
				columnNames, whereClause, whereArgs);
		return result;
	}

	public List<String> getAllSubCategoryNames(Context context) {
		Cursor result = helpDataCursor(context,
		...
		while (result.moveToNext()) {
			subCatNames.add(result.getString(result
					.getColumnIndex(Keys.SubCategory.NAME)));
		}
		return subCatNames;
	}

	public Pair<List<String>, List<Integer>> getSubCatNamesAndIDsLists(
			Context context) {
		Cursor result = helpDataCursor(context, new String[] {
				Keys.SubCategory.NAME, Keys.SubCategory.ID }, null, null);
		// DatabaseHandler db = new DatabaseHandler(context);
		// Cursor result = db.queryFromTable(DatabaseHandler.SUB_CATEGORY,
		// new String[]{Keys.SubCategory.NAME, Keys.SubCategory.ID}, null,
		// null);
		List<String> subCatNames = new ArrayList<String>();
	...
	public int getParentCategoryIDForSubCategoryID(Context context,
			int subCategoryID) {
		Cursor cursor = helpDataCursor(context,
				new String[] { Keys.SubCategory.PARENT_CATEGORY_ID },
				Keys.SubCategory.ID + "=?",
...
//				new String[] { Integer.toString(subCategoryID) });
		...
		return cursor.getInt(cursor
				.getColumnIndex(Keys.SubCategory.PARENT_CATEGORY_ID));
	}
...
//						+ "=?", new String[] { Integer.toString(ID) });
		...
		String questionsJSON = cursor.getString(cursor
				.getColumnIndex(Keys.SubCategory.PLAN));
	...
	public List<Pair<String, Integer>> getSubCategoryNamesAndIDsForParentCategory(
			Context context, int parentCategoryID) {
		Cursor result = helpDataCursor(context,new String[] { Keys.SubCategory.NAME, Keys.SubCategory.ID },
				Keys.SubCategory.PARENT_CATEGORY_ID + "=?",
...
//				new String[] { Integer.toString(parentCategoryID) });
	...
	public void addSubCategoriesFromJSON(Context context, String json) {
		try {
			...
			for (int index = 0; index < subCategories.length(); ++index) {
				...
				int parentCategoryID = subCategory
						.getInt(Keys.SubCategory.PARENT_CATEGORY_ID);
	...
	public void deleteSubCategory(Context context, int ID) {
		DatabaseHandler db = new DatabaseHandler(context);
		db.deleteEntries(DatabaseHandler.SUB_CATEGORY, Keys.SubCategory.ID
				+ "=?", new String[] { Integer.toString(ID) });
	}

	public String getTimeOfLastUpdate(Context context) {
		return TetherUtils.getTimeOfLastUpdate(context,
				DatabaseHandler.SUB_CATEGORY);
	}

	public void setTimeOfLastUpdate(Context context, String time) {
		TetherUtils.setTimeOfLastUpdate(context, DatabaseHandler.SUB_CATEGORY,
				time);
	}
